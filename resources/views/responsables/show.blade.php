        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg card-ies p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ $responsable->nombre }} {{ $responsable->apellidos }}</p>
                        </div>
                        
                        @if($responsable->cargo)
                        <div>
                            <p class="text-sm font-medium text-gray-500">Cargo</p>
                            <p class="text-gray-800 font-bold uppercase tracking-wider text-xs">
                                <span class="bg-ies-blue-50 text-ies-blue-700 px-2 py-1 rounded border border-ies-blue-100">
                                    {{ $responsable->cargo }}
                                </span>
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-800 font-medium">{{ $responsable->dni }}</p>
                        </div>

                        <div>
                            <p class="text-sm font-medium text-gray-500">Correo Electrónico</p>
                            <a href="mailto:{{ $responsable->email }}" class="text-ies-blue-600 hover:underline flex items-center">
                    ...
                    <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 self-start">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                                ...
                                <p class="text-md font-semibold text-gray-700">{{ $responsable->created_at->format('d/m/Y') }}</p>
                            </div>
                ...
                <div class="mt-12 flex justify-start">
                    <form action="{{ route('responsables.destroy', $responsable) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este responsable? Esta acción no se puede deshacer.')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-red-500 hover:text-red-700 text-sm font-medium flex items-center transition duration-150">
                            ...
                            {{ __('Eliminar Responsable Permanentemente') }}
                        </button>
                    </form>
                </div>
            </div>
