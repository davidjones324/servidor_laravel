    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl p-6 card-ies border-t-4 border-ies-green-500">
                <form action="{{ route('tutores.store') }}" method="POST" x-data='{ 
                        selectedCursos: @json(old('cursos', [])),
                        grupos: @json(old('grupos', [])),
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                @foreach(['ASIR', 'DAM', 'SMR'] as $ciclo)
                                    <label class="relative flex items-center p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition">
                                        <input type="checkbox" name="ciclos[]" value="{{ $ciclo }}" 
                                            {{ is_array(old('ciclos')) && in_array($ciclo, old('ciclos')) ? 'checked' : '' }}
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="telefono" id="telefono" value="{{ old('telefono') }}" 
                                class="mt-1 block h-10 w-full border-gray-300 rounded-md shadow-sm focus:border-ies-green-500 focus:ring-ies-green-500 sm:text-sm transition">
