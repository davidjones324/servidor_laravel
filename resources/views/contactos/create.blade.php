<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Crear Nuevo Contacto de Empresa') }}
        </h2>
    </x-slot>
                ...
                <form action="{{ route('contactos.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            @php
                                $selected_empresa = $empresas->firstWhere('id', old('empresa_id') ?? $selected_empresa_id);
                            @endphp
                            <div class="mt-1 block w-full bg-gray-50 border border-gray-200 rounded-md py-2 px-3 text-sm text-gray-600 font-bold">
                                {{ $selected_empresa ? $selected_empresa->razon_social : 'No seleccionada' }}
                            </div>
                            <input type="hidden" name="empresa_id" value="{{ old('empresa_id') ?? $selected_empresa_id }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="puesto" id="puesto" value="{{ old('puesto') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
